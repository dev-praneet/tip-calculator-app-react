...
const Span: React.FC<{ tax: string }> = props => {
  return <SpanContainer data-tax={props.tax}>{props.tax + '%'}</SpanContainer>;
};
...
const TaxRates: React.FC<{ taxPercent: string; setTaxPercent: React.Dispatch<React.SetStateAction<string>> }> = props => {
  ...
  // TODO using declaration merging to update the type of EventTarget is not working. Need to fix that.
